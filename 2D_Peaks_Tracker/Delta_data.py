...
class DatFile:
    ...
    # __init__ function run everytime you create instance of this class
    def __init__(self, filename):
        ...
        # open filename as f in read mode ('r')
        with open(filename, 'r') as f: 
            for line in f:
                line = line.rstrip('\n\t\r')  #strips from the right end of the line  
                
                # if the line starts with Timestamp, go to the ':' and split it into two, 
                # and store the second part as an attribute to that object
                ...
                if line.startswith('#\tname'):
                    name = line.split(': ', 1)[1]
                    # appends it to the columns property of the DatFile object
                    self.columns.append(name) 
                    
                elif line.startswith('#\tsize'):
                    size = int(line.split(': ', 1)[1])
                    # stores pairs of {name: size} in the sizes dictionary
...
#                         pass
        ...
        # takes the DatFile and creates a property for it which is its rePresentation as pandas DataFrame
        # ignoring all the lines that starts with # and understands that all the data are separated by tabs
        # and the names are the column names that were identified in the __init__ function before
    ...
    def save(self, filename):
        with open(filename, 'a') as f:
            ...
            for i in range(self.df.shape[1]):
                ...
                if i < 3:
                    f.write('#\tsize: %d\n' % self.sizes[self.df.columns[i]])
                    f.write('#\ttype: coordinate\n')
                else:
                    f.write('#\ttype: value\n')

            f.write('\n')

            self.df.to_csv(f, sep='\t', float_format='%.12e', header=False, index=False)
...
class Data2D:
    ...
    def interp_x(self, points):
        ...
        for i in range(rows):
            ...
            f = interpolate.interp1d(self.x[i], self.z[i], kind = 'linear', bounds_error = False, fill_value = np.nan, assume_sorted = False)
            values[i] = f(x)
